...
# List of elements to check for NULL/EMPTY values
ELEMENTS_TO_CHECK = {
    'Date', 'Name', 'Amount', 'Price', 'CurrCode', 'Status', 'Type', 'Mech', 'CustId',
    'AssSeqNo', 'PackId', 'ModelId', 'ElemId', 'ElemVers', 'Ty', 'Id', 'SN', 'PRO',
    'AccUsage', 'OrigVal', 'DiscVal', 'UM', 'Num', 'Main', 'NumStart', 'NumEnd', 'CoId',
    'SP', 'TM', 'SNDES', 'File', 'DocType', 'BillingMode', 'InvType', 'NumItem',
    'ArticleString', 'CT', 'ItemType', 'Days', 'Scale', 'MechId', 'Result', 'Finished',
    'ApplValue', 'ApplType', 'New', 'Sum', 'Max', 'Exp', 'BonPnt', 'Entity', 'Action',
    'PKey', 'ShDes', 'LongDes', 'CustomerId', 'CustCode', 'PartyType', 'MRKT', 'SM',
    'LZString', 'NumItems', 'HistId', 'FUP', 'FUE', 'FUEV', 'No', 'AdjAmt', 'Reason',
    'Comment', 'SeqNo', 'BundId', 'BundVers', 'BPROD', 'Desc', 'Value', 'Sender', 'BAId',
    'Text', 'AS'
}
            ...
            with mmap.mmap(xml_file.fileno(), 0, access=mmap.ACCESS_READ) as mm:
                ...
                try:
                    for event, elem in context:
                        if event == 'start':
                            if elem.tag in ELEMENTS_TO_CHECK:
                            ...
                            if 'lang' in elem.attrib and elem.attrib['lang'].strip() == "":
                                null_errors.append(f"Empty value in 'xml:lang' attribute of element '{elem.tag}'")
...
def main():
    try:
        ...
        4. The app will:
            ...
            - Check for NULL/empty values in specified elements and attributes
            - Handle large files (up to 400MB)
        5. Validation rules:
            - XML structure must be well-formed
            - No empty/NULL values allowed in specified elements and attributes
            - Files with NULL values in specified elements will be marked as invalid
        6. Results show:
            - Invalid files first
